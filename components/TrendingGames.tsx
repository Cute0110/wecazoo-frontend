...
const TrendingGames = ({ allGamesData, gameSectionType, sectionTitle }: any) => {
  ...
  const scrollLeft = () => {
    if (scrollAreaRef.current) {
      ...
      scrollAreaRef.current.scrollBy({
        left: -100, // Adjust the value for how far you want to scroll
        behavior: "smooth",
      });
      scrollAreaRef.current.scrollLeft = -10;
    }
  };
      ...
      <section className="container my-12">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center justify-end gap-4 px-4">
            <Image src="/images/leftArrow.jpg" className="rounded-md cursor-pointer" onClick={scrollLeft} alt={sectionTitle} width={40} height={40} />
            <Image src="/images/rightArrow.jpg" className="rounded-md cursor-pointer" onClick={scrollRight} alt={sectionTitle} width={40} height={40} />
          </div>
        </div>

        <ScrollArea className="w-full whitespace-nowrap rounded-[10px] p-4">
          {games.length > 0 ? (
            <div ref={scrollAreaRef} className="flex w-max space-x-4 p-3 overflow-x-auto">
